...
BASE_URL="${BASE_URL:-https://ftp.ncbi.nlm.nih.gov/pubmed/baseline}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DEST_DIR="${1:-$ROOT/data/raw/pubmed/baseline}"
PARALLEL="${PARALLEL:-4}"
...
echo "[1/5] Downloading baseline index and README into $DEST_DIR"
...
if [[ ! -s "$FILES_TXT" ]]; then
  echo "No PubMed baseline files were found in $INDEX_HTML" >&2
  exit 1
fi
...
echo "Tip: verify a sample with: cd \"$DEST_DIR\" && md5 -r pubmed26n0001.xml.gz"
